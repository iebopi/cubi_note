#!/bin/sh

echo 1 > /dev/watchdog

source script_parser.sh

if [ ! -d /system/vendor/ ]; then
    mkdir -p /system/vendor/
    ln -s /lib/modules/`uname -r`/ /system/vendor/modules
    ln -s /boot/*.hcd /system/vendor/modules/
    ln -s /boot/*.bin /system/vendor/modules/
    ln -s /boot/*.txt /system/vendor/modules/
fi

ROOT_DEVICE="/dev/mmcblk0p7"
for parm in $(cat /proc/cmdline); do
    case $parm in
        root=*)
            ROOT_DEVICE=`echo $parm | awk -F\= '{print $2}'`
            ;;  
    esac
done

# install nand driver if we boot from sdmmc
nand_activated=`script_fetch "nand" "activated"`
if [ $nand_activated -eq 1 ]; then
    case $ROOT_DEVICE in
        /dev/mmc*)
  	    
        nand_module_path=`script_fetch "nand" "module_path"`
        if [ -n "$nand_module_path" ]; then
     	  	 insmod "$nand_module_path"
  	    fi
            ;;
    esac

fi

insmod /lib/modules/`uname -r`/disp.ko
insmod /lib/modules/`uname -r`/lcd.ko
insmod /lib/modules/`uname -r`/hdmi.ko
insmod /lib/modules/`uname -r`/ft5x_ts.ko
insmod /lib/modules/`uname -r`/sunxi-ts.ko

# insmod ir driver
ir_activated=`script_fetch "ir" "activated"`
if [ $ir_activated -eq 1 ]; then
    ir_module_path=`script_fetch "ir" "module_path"`
    if [ -n "$ir_module_path" ]; then
        insmod "$ir_module_path"
    fi
fi

# start camera test firstly
while true; do
    camera_activated=`script_fetch "camera" "activated"`
    echo "camera activated #$camera_activated"
    if [ $camera_activated -eq 1 ]; then
        echo "camera activated"
        module_count=`script_fetch "camera" "module_count"`
        if [ $module_count -gt 0 ]; then
            for i in $(seq $module_count); do
                key_name="module"$i"_path"
                module_path=`script_fetch "camera" "$key_name"`
                if [ -n "$module_path" ]; then
                    insmod "$module_path"
                    if [ $? -ne 0 ]; then
                        echo "insmod $module_path failed"
                        break 2
                    fi
                fi
            done
        fi
    else
        echo "camera not activated"
        break
    fi

    echo "camera module insmod done"
    touch /tmp/camera_insmod_done
done

# insmod touchscreen driver
tp_module_path=`script_fetch "tp" "module_path"`
if [ -n "$tp_module_path" ]; then
    insmod "$tp_module_path"

    # calibrate touchscreen if need
    tp_type=`script_fetch "tp" "type"`
    if [ $tp_type -eq 0 ]; then
        while true; do
            ts_calibrate
            if [ $? -eq 0 ]; then
                break
            fi
        done
    fi

    #ts_test
else
    echo "NO!!! touchscreen driver to be insmod"
fi

(echo "root"
sleep 1
echo "root") | passwd root
chown root /var/empty
/usr/local/sbin/sshd
echo /sbin/mdev > /proc/sys/kernel/hotplug
mdev -s

wpa_supplicant -i wlan0 -c /etc/wpa_supplicant.conf &
#sleep 4
#udhcpc -i wlan0 -n -q -t 6
sh /etc/init.d/auto_config_network
#if [ ! -e /etc/pointercal ]; then
#        ts_calibrate
#fi
 
rm /var/lock/* -r

if [ -f /opt/LoaderManagerUpdate ]
then
	rm /opt/LoaderManager
	mv /opt/LoaderManagerUpdate /opt/LoaderManager
fi
sync

chmod 777 /opt/*
cd /opt
./LoaderManager -platform directfb &
sleep 3
#GPIO
echo 184 > /sys/class/gpio/export
echo in > /sys/class/gpio/gpio184/direction
echo 185 > /sys/class/gpio/export
echo in > /sys/class/gpio/gpio185/direction
echo 182 > /sys/class/gpio/export
echo in > /sys/class/gpio/gpio182/direction
echo 193 > /sys/class/gpio/export
echo in > /sys/class/gpio/gpio193/direction
echo 181 > /sys/class/gpio/export
echo in > /sys/class/gpio/gpio181/direction
#COM W/R CONTROLL
# ttyS5 rx/tx
echo 176  > /sys/class/gpio/export
echo out > /sys/class/gpio/gpio176/direction
echo 0 > /sys/class/gpio/gpio176/direction
# ttyS4 rx/tx
echo 32  > /sys/class/gpio/export
echo out > /sys/class/gpio/gpio32/direction
echo 0 > /sys/class/gpio/gpio32/direction

# audio enable
echo 187  > /sys/class/gpio/export
echo out > /sys/class/gpio/gpio187/direction
echo 1 > /sys/class/gpio/gpio187/value
#blacklight on
echo 179 > /sys/class/gpio/export
echo out > /sys/class/gpio/gpio179/direction
echo 0 > /sys/class/gpio/gpio179/value
#set light
/opt/UpdateLight.sh

dos2unix /opt/KillApp.sh
dos2unix /opt/StartApp.sh

chmod 777 /opt/Release/SHScreen
cd /opt/Release
./SHScreen -platform directfb &

echo V > /dev/watchdog
