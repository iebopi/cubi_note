...
) ENGINE=InnoDB AUTO_INCREMENT=251 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='操作权限表';
...
CREATE TABLE `tb_material_template` (
  ...
  `version_flag` int(3) DEFAULT '4' COMMENT '3迁移数据（v3.0），4新版数据（v4.0）',
  PRIMARY KEY (`id`)
...
CREATE TABLE `tb_play_list` (
  ...
  `gmt_modified` datetime DEFAULT NULL COMMENT '修改时间',
  `material_Ids` text COMMENT '所有素材的ID',
