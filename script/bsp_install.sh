# !/bin/bash
ehco "Ensure Login as root!"
echo "Type your choice, the default value is no."
read -p "apt update?(Y/n):" Val
case $Val in
Y/y)
apt-get update
;;
...
read -p "bsp install including gcc g++?(Y/n):" Val
case $Val in
Y|y)
...
#修改默认gcc为gcc-4.8
read -p "switch gcc version to 4.8/4.7?(Y/n):" Val
case $Val in
Y|y)
...
read -p "useful tools install?(Y/n):" Val
case $Val in
Y|y)
